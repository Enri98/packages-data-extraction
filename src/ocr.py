...
def extract_ocr_words(pdf_path: Path) -> list[tuple[str, float]]:
    """
    Return a list of (word, confidence) tuples from OCR results on all pages.

    Useful for future spatial / confidence-weighted logic. Unlike
    :func:`extract_ocr_text`, results here are NOT cached because the word-level
    detail is not needed by default and would double memory use.

    Args:
        pdf_path: Path to the PDF file to process.

    Returns:
        List of (word_token, confidence) tuples in reading order.
    ...
    try:
        ...
        raise RuntimeError("pypdfium2 is required for OCR extraction.") from exc
        ...
        if result:
            for _box, text, conf in result:
                for token in text.split():
                    words.append((token, float(conf)))

    logger.debug("extract_ocr_words | path={} | word_count={}", resolved, len(words))
    return words
